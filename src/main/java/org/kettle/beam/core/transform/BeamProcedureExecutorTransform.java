...
public class BeamProcedureExecutorTransform extends PTransform<PCollection<KettleRow>, PDone> {
    ...
    private static final Logger LOG = LoggerFactory.getLogger( BeamProcedureExecutorTransform.class );
    private static final Counter numErrors = Metrics.counter( "main", "BeamProcedureExecutorError" );
    private static final Counter numUpdate = Metrics.counter( "main", "BeamProcedureExecutor" );

    public BeamProcedureExecutorTransform() {}

    public BeamProcedureExecutorTransform(String stepname
            , String databaseType
            , String driver
        ...
        try {
            ...
            return input.apply(JdbcIO.<KettleRow>write()
                    ...
                    .withPreparedStatementSetter((element, preparedStatement) -> {
                        BeamProcedureExecutorTransform.this.setPreparedStatement(element, preparedStatement);
                    }));
        } catch (Exception e) {
            numErrors.inc();
            LOG.error( "Error in Beam Procedure Executor transform", e );
            throw new RuntimeException( "Error in Beam Procedure Executor transform", e );
        }
    }
